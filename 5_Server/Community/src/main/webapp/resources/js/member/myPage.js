...
function infoValidate() {
	const updateNick = document.getElementById("memberNickname");
	const updateTel = document.getElementById("memberTel");
	if (updateNick.value.trim() === "") {
		alert("닉네임을 입력해주세요.");
		updateNick.focus();
		return false;
	}
	if (updateTel.value.trim() === "") {
		alert("전화번호를 입력해 주세요.");
		updateTel.focus();
		return false;
	} else {
...
function changePwValidate() {
	...
	if (newPw.value.trim() !== newPwConfirm.value.trim()) {
