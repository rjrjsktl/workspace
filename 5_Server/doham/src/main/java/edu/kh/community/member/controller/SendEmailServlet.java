...
public class SendEmailServlet extends HttpServlet {
   @Override
   protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException { // 단순   조회라서 get을   사용중.

      String inputEmail = req.getParameter("inputEmail"); // 입력받은 이메일 파라미터 가져오기.
      String subject = "[Commnity 프로젝트] 회원 가입 이메일 인증번호"; // 제목

      String fromEmail = "dogeunlee89@gmail.com"; // 보내는 사람으로 표시될 이메일 (이메일 따라서 안될수도 있음)
      ...
      final String smtpEmail = "dogeunlee89@gmail.com"; // 이메일
      final String password = "cxnejjswhpixcbni"; // 발급 받은 비밀번호
      // 메일 옵션 설정
      ...
      try {
         ...
         // sysdate
         // 인증번호를 받은 이메일, 인증번호, 인증번호 발급 시간 -> DB 삽입 // INSERT SQL문을 사용하며, INSERT는 행의 갯수로 넘어온다. 그래서 INT를 사용한다.
         int result = new MemberService().insertCertification(inputEmail, cNumber);
         resp.getWriter().print(result); // 응답용 객체 resp
